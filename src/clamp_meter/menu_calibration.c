...
float32_t
calibration_clamp_pos_refsel(void)
{
...
void
calibration_clamp_pos_display_data(void)
{
    ...
    case CLAMP_CAL_VOUT: LCD_write_float2(clamp_measurements_result.V_ovrl, 5); break;

    case CLAMP_CAL_SHUNT: LCD_write_float2(clamp_measurements_result.V_shunt, 5); break;

    case CLAMP_CAL_CLAMP: LCD_write_float2(clamp_measurements_result.I_clamp, 5); break;
    }
}

void
calibration_clamp_pos_terminate(void)
{
...
void
calibration_clamp_pos_calculate(void)
{
    measurement_stop();
    switch_sensing_chanel(VOLTAGE_SENSOR);

    Clamp_calibrator.I_clamp_delta =
      clamp_measurements_result.I_clamp * Clamp_calibrator.normalize_coeff - Clamp_calibrator.I_clamp_orig;
    Clamp_calibrator.position_gain = Clamp_calibrator.I_clamp_delta_calc / Clamp_calibrator.I_clamp_delta;
...
void
calibration_clamp_pos_clamp(calibrator_action_type_t action)
{
    if (action == CALIBRATOR_GO_NEXT_STEP) {
        Clamp_calibrator.normalize_coeff = Clamp_calibrator.V_applied_1 / clamp_measurements_result.V_applied;
...
void
calibration_clamp_pos_shunt(calibrator_action_type_t action)
{
    if (action == CALIBRATOR_GO_NEXT_STEP) {
...
void
calibration_clamp_pos_vout(calibrator_action_type_t action)
{
    if (action == CALIBRATOR_GO_NEXT_STEP) {
        LCD_cursor_setpos(1, 1);
...
void
calibration_clamp_pos_connect_ref(calibrator_action_type_t action)
{
    if (action == CALIBRATOR_GO_NEXT_STEP) {
        ...
        Clamp_calibrator.I_clamp_delta_calc   = Clamp_calibrator.V_applied_1 / Clamp_calibrator.reference_resistance;

        LCD_cursor_setpos(1, 1);
        LCD_write("                    ");
        LCD_cursor_setpos(1, 1);
        LCD_write("Connect " conv_to_str(Clamp_calibrator.reference_resistance) "Ohm");
        LCD_cursor_setpos(1, 2);
...
void
calibration_clamp_pos_hellopage(void)
{
    LCD_cursor_setpos(1, 1);
    LCD_write("Calibrate           ");
    LCD_cursor_setpos(1, 2);
...
void
calibration_clamp_position(uint16_t key)
{
    ...
    switch (Clamp_calibrator.phase) {
    case CLAMP_CAL_START: calibration_clamp_pos_hellopage(); break;

    case CLAMP_CAL_HELLOPAGE: calibration_clamp_pos_connect_ref(action); break;

    case CLAMP_CAL_CONNECT_REFERENSE: calibration_clamp_pos_vout(action); break;

    case CLAMP_CAL_VOUT: calibration_clamp_pos_shunt(action); break;

    case CLAMP_CAL_SHUNT: calibration_clamp_pos_clamp(action); break;

    case CLAMP_CAL_CLAMP: calibration_clamp_pos_calculate(); break;
    }
}

void
calibration_terminate(void)
{
...
calibration_start(void)
{
    ...
    default: Calibrator.error_occured = true;
    }
}

void
calibration_pause(void)
{
    Calibrator.is_calibrating = false;
    measurement_stop();
}
...
void
calibration_reset_variables(void)
{
    Calibrator.new_data_is_ready = false;
...
void
calibration_display_measured_data(void)
{
    float32_t data;
    ...
    if ((Calibrator.cal_phase == CALIBRATION_MEASURE_VOUT_G0) || (Calibrator.cal_phase == CALIBRATION_MEASURE_VOUT_G1) ||
        (Calibrator.cal_phase == CALIBRATION_MEASURE_VOUT_G2) || (Calibrator.cal_phase == CALIBRATION_MEASURE_VOUT_G3) ||
...
void
calibration_write_phase_header(uint8_t phase_num)
{
    LCD_cursor_setpos(1, 1);
...
void
calibration_semiauto_clamp_gX(calibrator_action_type_t action)
{
    calibration_pause();
    ...
    if (action == CALIBRATOR_GO_NEXT_STEP) {
        ...
        if (Calibrator.cal_phase_num == 1) {
            Calibrator.clamp_gains[Calibrator.cal_phase_num - 1] =
              Calibrator.sensor_mag * 3 / (Calibrator.v_sh / R_SHUNT);
        }
        else {
            Calibrator.clamp_gains[Calibrator.cal_phase_num - 1] = Calibrator.sensor_mag / (Calibrator.v_sh / R_SHUNT);
        }

        Calibrator.clamp_angles[Calibrator.cal_phase_num - 1] = Calibrator.sensor_phi - Calibrator.vout_phi;
        ...
        switch (Calibrator.cal_phase) {
        case CALIBRATION_CLAMP_G0: LCD_write(conv_to_str(CALIBRATION_G1_R) " Ohm"); break;

        case CALIBRATION_CLAMP_G1: LCD_write(conv_to_str(CALIBRATION_G2_R) " Ohm"); break;

        case CALIBRATION_CLAMP_G2: LCD_write(conv_to_str(CALIBRATION_G3_R) " Ohm"); break;

        case CALIBRATION_CLAMP_G3: LCD_write(conv_to_str(CALIBRATION_G4_R) " Ohm"); break;
...
void
calibration_semiauto_shunt_gX(calibrator_action_type_t action)
{
    calibration_pause();
    ...
    if (action == CALIBRATOR_GO_NEXT_STEP) {
        ...
        Calibrator.shunt_gains[Calibrator.cal_phase_num - 1] = Calibrator.sensor_mag / Calibrator.v_sh;

        Calibrator.shunt_angles[Calibrator.cal_phase_num - 1] = Calibrator.sensor_phi - Calibrator.vout_phi;
...
void
calibration_semiauto_measure_vout_gX(calibrator_action_type_t action)
{
    calibration_pause();
    ...
    if (action == CALIBRATOR_GO_NEXT_STEP) {
        ...
        Calibrator.vout_ovrl = Calibrator.sensor_mag / Calibrator.v_sens_gain;

        Calibrator.vout_phi = Calibrator.sensor_phi - Calibrator.vout_phi_noload;

        switch (Calibrator.cal_phase_num) {
        case 1: test_resistance = CALIBRATION_G0_R; break;

        case 2: test_resistance = CALIBRATION_G1_R; break;

        case 3: test_resistance = CALIBRATION_G2_R; break;

        case 4: test_resistance = CALIBRATION_G3_R; break;

        case 5: test_resistance = CALIBRATION_G4_R; break;
        default: test_resistance = 0;
        }

        Calibrator.v_sh = Calibrator.vout_ovrl * R_SHUNT / (test_resistance + R_SHUNT);
...
void
calibration_semiauto_set_gX(calibrator_action_type_t action)
{
    if (action == CALIBRATOR_GO_NEXT_STEP) {
        //		calibration_write_phase_header(Calibrator.cal_phase_num);
        //
...
void
calibration_semiauto_warning_g0(calibrator_action_type_t action)
{
    if (action == CALIBRATOR_GO_NEXT_STEP) {
        //		LCD_cursor_setpos(1, 1);
        //		LCD_write("Connect             ");
...
void
calibration_semiauto_helper_input_vout_mesd(calibrator_action_type_t action)
{
    LCD_cursor_disable();
    ...
    if (action == CALIBRATOR_GO_NEXT_STEP) {
        ...
        Calibrator.v_sens_gain = Calibrator.sensor_mag / Calibrator.externally_measured_vout;
...
void
calibration_semiauto_helper_vout(calibrator_action_type_t action)
{
    calibration_pause();
...
void
calibration_semiauto_helper_vout_warning_shown(calibrator_action_type_t action)
{
    if (action == CALIBRATOR_GO_NEXT_STEP) {
        //		LCD_cursor_setpos(1, 1);
...
void
calibration_semiauto_helper_start(calibrator_action_type_t action)
{
    if (action == CALIBRATOR_GO_NEXT_STEP) {
        //		LCD_cursor_setpos(1, 1);
...
void
calibration_semiauto_helper_hellopage(void)
{
...
void
calibration_semiauto(calibrator_action_type_t action)
{
    switch (Calibrator.cal_phase) {
    case CALIBRATION_HELLO_PAGE: calibration_semiauto_helper_hellopage(); break;

    case CALIBRATION_START: calibration_semiauto_helper_start(action); break;

    case CALIBRATION_VOUT_MSG_SHOWN: calibration_semiauto_helper_vout_warning_shown(action); break;

    case CALIBRATION_VOUT: calibration_semiauto_helper_vout(action); break;

    case CALIBRATION_GET_VOUT_MEASURED_EXT: calibration_semiauto_helper_input_vout_mesd(action); break;

    case CALIBRATION_WARNING_G0: calibration_semiauto_warning_g0(action); break;

    case CALIBRATION_SET_G0R:
    case CALIBRATION_SET_G1R:
    case CALIBRATION_SET_G2R:
    case CALIBRATION_SET_G3R:
    case CALIBRATION_SET_G4R: calibration_semiauto_set_gX(action); break;

    case CALIBRATION_MEASURE_VOUT_G0:
    case CALIBRATION_MEASURE_VOUT_G1:
    case CALIBRATION_MEASURE_VOUT_G2:
    case CALIBRATION_MEASURE_VOUT_G3:
    case CALIBRATION_MEASURE_VOUT_G4: calibration_semiauto_measure_vout_gX(action); break;

    case CALIBRATION_SHUNT_G0:
    case CALIBRATION_SHUNT_G1:
    case CALIBRATION_SHUNT_G2:
    case CALIBRATION_SHUNT_G3:
    case CALIBRATION_SHUNT_G4: calibration_semiauto_shunt_gX(action); break;

    case CALIBRATION_CLAMP_G0:
    case CALIBRATION_CLAMP_G1:
    case CALIBRATION_CLAMP_G2:
    case CALIBRATION_CLAMP_G3:
    case CALIBRATION_CLAMP_G4: calibration_semiauto_clamp_gX(action); break;

    case CALIBRATION_CONFIRM: calibration_save(); break;
    }
}
