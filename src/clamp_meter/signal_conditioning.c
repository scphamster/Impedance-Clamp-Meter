...
void shunt_sensor_set_gain(uint8_t gain)
{
	switch (gain) {
	case 0: {
		pio_pull_down(PIOD, SH_SENSOR_GAIN_B_PIN | SH_SENSOR_GAIN_C_PIN |
		              SH_SENSOR_GAIN_D_PIN, 0);
...
void clamp_sensor_set_gain(uint8_t gain)
{
	...
	case 0: {
		...
		pio_pull_up(PIOA, CLAMP_SENSOR_GAIN_B_PIN | CLAMP_SENSOR_GAIN_C_PIN |
		            CLAMP_SENSOR_GAIN_D_PIN, false);
...
void composite_gain_controll(sensor_type_t sensor)
{
	if (sensor == SHUNT_SENSOR) {
		...
		if (Analog.shunt_sensor_gain !=
		    shunt_sensor_gain_preset[Analog.overall_gain]) {
		...
		if (Analog.clamp_sensor_gain !=
		    clamp_sensor_gain_preset[Analog.overall_gain]) {
...
void decrease_gain(void)
{
	...
	if ((Analog.selected_sensor != SHUNT_SENSOR) &&
	    (Analog.selected_sensor != CLAMP_SENSOR)) {
		Analog.error_occured = true;
		return;
	}
...
void recall_coeffs_from_flash_struct(void)
{
	...
	if (end_storage->data_password == CALIBRATION_DATA_PASSWORD) {
		...
		for (idx = 0; idx < SHUNT_SENSOR_GAIN_COEFFS_NUM; idx++) {
			Cal_data.shunt_gain[idx] =
			end_storage->shunt_gain[idx];
		...
		for (idx = 0; idx < CLAMP_SENSOR_GAIN_COEFFS_NUM; idx++) {
			Cal_data.clamp_gain[idx] =
			  end_storage->clamp_gain[idx];
		...
		for (idx = 0; idx < CLAMP_SENSOR_GAIN_COEFFS_NUM; idx++) {
			Cal_data.clamp_gain[idx] =
			  Reserve_cal_data.clamp_gain[idx];
		...
		for (idx = 0; idx < SHUNT_SENSOR_GAIN_COEFFS_NUM; idx++) {
			Cal_data.shunt_gain[idx] =
			Reserve_cal_data.shunt_gain[idx];
