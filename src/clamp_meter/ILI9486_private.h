...
#define ILI_WR_STROBE		TFT_CTRL_PORT&=~WR_PIN; TFT_CTRL_PORT|=WR_PIN			//strobe to write/read data to/from LCD
...
void TFT_SetAddrWindow		(int16_t x_beg, int16_t y_beg, int16_t x_end,
							 int16_t y_end);
void TFT_fillCircleHelper	(int16_t x0, int16_t y0, int16_t r,
							uint8_t cornername, int16_t delta, uint16_t color);
void TFT_Print_Char			(uint16_t x, uint16_t y, char data , uint8_t mode,
							uint8_t size);
...
extern uint8_t font_offset;
extern uint8_t *font_addr;
extern uint16_t tft_W;
extern uint16_t tft_H;
extern int16_t   _width, _height, cursor_x, cursor_y , char_color ;
...
/*********************************************************************
 *
 *  ORINTIONS OF SCREEN ORIENTATION
 *
 ***********************************************************************/
