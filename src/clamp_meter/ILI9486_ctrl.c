...
static inline void
tft_noreset_set(void)
{
    pio_set(TFT_RESET_PIO, TFT_RESET_PIN);
}

static inline void __attribute__((hot)) tft_write_strobe(void)
{
    tft_write_set();
    // delay_us(10);
    tft_nowrite_set();
...
static inline void __attribute__((hot)) tft_data_port1(uint8_t data)
{
    uint32_t data_a = 0;
    uint32_t data_d = 0;
...
static inline void __attribute__((hot)) tft_data_port2(uint8_t data)
{
    uint32_t data_a = 0;
    uint32_t data_b = 0;
    uint32_t data_d = 0;
...
static inline void
TFT_Write_Cmd_Data(uint16_t cmd, uint16_t data)
{
...
static inline void
TFT_Write_Data_Cmd(uint16_t cmd, uint16_t data)
{
    TFT_Write_Cmd_Byte(cmd);
    TFT_Write_Data_Word(data);
}
...
static inline void
TFT_Write_Cmd_ParamN(uint8_t cmd, int8_t N, uint8_t *block)
{
    TFT_Write_Cmd_Byte(cmd);
...
static inline void
TFT_Write_Cmd_Param4(uint8_t cmd, uint8_t d1, uint8_t d2, uint8_t d3, uint8_t d4)
{
    uint8_t d[4];
...
void
TFT_Scroll_Vertical(int16_t top, int16_t scrollines, int16_t offset)
{
...
void
TFT_SetAddrWindow(int16_t x_beg, int16_t y_beg, int16_t x_end, int16_t y_end)
{
...
void
TFT_text_color_set(uint16_t pixelcolor, uint16_t backcolour)
{
...
void
TFT_cursor_set(int16_t x, int16_t y)
{
    cursor_x = x;
    cursor_y = y;
}
...
void
TFT_put_N_pixels(unsigned int color, uint32_t n)
{
...
void
TFT_frect_print(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color)
{
    int16_t  end;
    uint32_t n_pixels;
...
void
TFT_Draw_Line(int16_t x1, int16_t y1, int16_t x2, int16_t y2, uint16_t color)
{
    int16_t i;
    int16_t dx, dy;
    int16_t sx, sy;
...
void
TFT_Draw_Circle(int16_t x0, int16_t y0, int16_t r, uint16_t color)
{
    int16_t f     = 1 - r;
    int16_t ddF_x = 1;
    int16_t ddF_y = -2 * r;
...
void
TFT_fillCircle(int16_t x0, int16_t y0, int16_t r, uint16_t color)
{
...
void
TFT_Draw_Point(int16_t x0, int16_t y0, uint16_t color)
{
    int16_t r = 1;
    TFT_DrawFastVLine(x0, y0 - r, 2 * r + 1, color);
    TFT_fillCircleHelper(x0, y0, r, 3, 0, color);
...
void
TFT_fillCircleHelper(int16_t x0, int16_t y0, int16_t r, uint8_t cornername, int16_t delta, uint16_t color)
{
    int16_t f     = 1 - r;
    int16_t ddF_x = 1;
    int16_t ddF_y = -2 * r;
...
void
TFT_DrawRect(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color)
{
...
void
TFT_DrawFastVLine(int16_t x, int16_t y, int16_t h, uint16_t color)
{
...
void
TFT_DrawFastHLine(int16_t x, int16_t y, int16_t w, uint16_t color)
{
...
void
TFT_Print_Char(uint16_t x, uint16_t y, char data, uint8_t mode, uint8_t size)
{
...
void
TFT_print_str(const char *string, uint8_t string_mode, uint8_t size)
{
    uint8_t i      = 0;
...
void
TFT_print_number(uint16_t number, uint8_t TFT_STRING_MODE, uint8_t size)
{
    int16_t temp = 1;
