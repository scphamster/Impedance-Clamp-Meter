...
void
delete_last_data_from_buffer(void)
{
    uint16_t bytes_left;
    ...
    while (bytes_left) {
        twi_instance.data[data_counter] = twi_instance.data[data_counter + shift_size];
    ...
    do {
        ...
        twi_instance.device_addr[data_counter] = twi_instance.device_addr[data_counter + 1];
        data_counter++;
    } while ((twi_instance.data_len[data_counter - 1] != 0) && ((data_counter + 1) < DATABUFF_LEN));
}

void
twiPdc_write(uint8_t *write_data, uint8_t datalen, uint8_t devadr)
{
    ...
    while (data_counter < datalen) {
        twi_instance.data[twi_instance.bytes_occupied + data_counter] = write_data[data_counter];
    ...
    if ((twi_get_interrupt_mask(twi_module) == 0) && ((twi_instance.packets_occupied - 1) == 0))
        start_transfer();
}
